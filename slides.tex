...
\title[]{Evaluation of Numerical Approximations of the Stiffness Term for Shallow Water Equations}
...
\part{Evaluation of Numerical Approximations of the Stiffness Term for Shallow Water Equations}
...
\begin{frame}
  ...
  \begin{itemize}
  \item Wave simulation based on hyperbolic equations
  ...
  \item Last time: Computations along the edges of triangles
  \item Now: Computations within cells (numerical approximation)
  \end{itemize}
  \begin{center}
    \includegraphics[scale=1.25]{sample-screenshot2.png}
  \end{center}
\end{frame}

\begin{frame}
  \frametitle{Shallow Water Equations}
  \note{homogeneous form}
  \vspace{-.3cm}
  \begin{equation}
  ...
  \begin{columns}
    \begin{column}{.3\textwidth}
      \begin{flushright}
        \begin{tikzpicture}
          \draw[decorate, decoration={snake, amplitude=.25cm, segment length=.75cm}] (0,0) -- (2,0);
          \fill[fill=blue, fill opacity=0.2] (0, -1) -- (0,0)
          decorate[decoration={snake, amplitude=.25cm, segment length=.75cm}] {(0, 0) -- (2, 0)}
          -- (2, -1) -- (0,-1);
          \draw[decorate, decoration={brace}] (1.8, -1) --node[left]{$h$} (1.8, .2);
          \draw[>=latex, ->] (1.8,.2) -- (2.2, .2) node[right]{$\mathbf{u}$};
          \draw (0, -1) -- (2, -1);
        \end{tikzpicture}
      \end{flushright}
    \end{column}
    \begin{column}{.6\textwidth}
      \begin{itemize}
      \item $h$: Water height
      \item $\mathbf{u}$: Momentums in $x$ and $y$ direction
      \end{itemize}
    \end{column}
  \end{columns}
  ...
  \begin{columns}
    ...
    \begin{column}{.4\textwidth}
      ...
      \begin{tikzpicture}[scale=2]
        \draw (0,0) -- (1,0);
        \draw[dotted] (1,0) -- (0.6,0.3) -- (0,0);
        \begin{scope}[yshift=.5cm]
          \draw (0,0) -- (1,0) -- (0.6,0.3) -- (0,0);
        \end{scope}
        \draw (0,0) -- (0,0.5);
        \draw (1,0) -- (1,0.5);
        \draw[dotted] (.6,0.3) -- (.6,.8);
        \draw[>=latex, ->] (0.5,0.65) -- (1.2,1.) node[right]{\small$\begin{pmatrix}u_x\\u_y\end{pmatrix}$};
        \draw[xshift=.1cm,decorate, decoration={brace, amplitude=3pt}]
        (1,0.5) -- node[right]{$h$} (1, 0);
      \end{tikzpicture}
    \end{column}
  \end{columns}
...
\newcommand{\qapprox}{\hat{\mathbf{q}}}
...
\begin{frame}
  ...
  \item A basis function $\phi_i$ for each support point $\rightarrow$ nodal basis
    % \begin{itemize}
    % \item Basis functions $\phi_i$: (two dimensional) Lagrange polynomials
    % \end{itemize}
  \item Approximate
    $\mathbf{q} \approx \sum_{i=1}^n \mathbf{q}_i \phi_i(x,y) =: \qapprox$
    \begin{itemize}
    \item Introduces error (w.r.t. reality) since flux function not linear
    %\item For this presentation, we call this approximation \emph{``exact analytic solution''}
    \item $\mathbf{q}_i$ contains height and momentums for $i$-th support point
    \item $\mathbf{q}_i$ independent of $x$ and $y$ $\rightarrow$
      %(in contrast to $F(\mathbf{q})$) $\rightarrow$ 
      allows matrix formulations
    \end{itemize}
  \end{itemize}

  \begin{equation}
    \label{eq:shallow-water-phi-i}
    \int_T \pd {\qapprox}{t} \phi_i \, dT
    - \underbrace {\int_T F(\qapprox) \cdot \nabla \phi_i \, dT}
    _{\text{stiffness term}}
    + \int_{\partial T} F(\qapprox) \cdot \mathbf{n} \phi_i \, ds
    = 0
  \end{equation}
\end{frame}

\begin{frame}
  \frametitle{Short excursion: matrices}
  \vspace{-.3cm}
  \begin{equation*}
    \underbrace{\int_T \pd {\qapprox}{t} \phi_i \, dT}_{\text{mass term}}
    - 
    \underbrace{\int_T F(\qapprox) \cdot \nabla \phi_i \, dT}_{\text{stiffness term}}
    + 
    \underbrace{\int_{\partial T} F(\qapprox) \cdot \mathbf{n} \phi_i \, ds}_{\text{flux term}}
    = 0
  \end{equation*}
  \begin{block}{\emph{Precomputable} matrices (independent of $x$ and $y$)}
    ...
    \item Mass matrix
      ...
      % \end{equation}
    \item Flux matrices (one for each edge $e$): last time %$\forall e \in E$:
      % \begin{equation}
...
\begin{frame}
  ...
  \begin{block}{Stiffness term accuracy analysis}
    \begin{itemize}
    \item Stiffness term computation involves integrals

      \dots over fractional functions
      $\rightarrow$ very complicated and expensive to compute
    \item Solution: Use approximation

      $\rightarrow$ \alert<2>{Question: How good is this approximation?}
    \item   Remark: Approximation exact for height

      $\rightarrow$ only consider momentum error
    \end{itemize}
  \end{block}
\end{frame}

\section{Approximate vs. ``exact'' result}
\label{sec:stiffness-exact-approx}

\begin{frame}
  \frametitle{Stiffness term --- sum approximation}
  \vspace{-.5cm}
  \begin{eqnarray*}
    \mathbf{q} =
    \begin{pmatrix}
      h \\ u_x \\ u_y
    \end{pmatrix}, F(\mathbf{q}) =
    {
      \renewcommand{\arraystretch}{1.2}
      \begin{pmatrix}
        \mathbf{u} \\ \frac{u_x}{h} \mathbf{u} + \frac{1}{2} g h^2 e_x \\ \frac{u_y}{h} \mathbf{u}+
  ...
  \begin{align*}
    \int_T F_2(\widehat{\mathbf{q}}) \cdot \nabla \phi_i\, dT = 
    \int_T F_2 \left( \sum_{j=1}^n \mathbf{q}_j \cdot \phi_j \right) \cdot \nabla \phi_i\, dT 
    % =:
    % \int_T \widehat{F}_2 \left( \mathbf{q} \right)\cdot \nabla \phi_i \, dT 
  \end{align*}

  \begin{itemize}
  \item Above computation too expensive (integral over fractional function)

    $\rightarrow$ Use sum approximation:
  \end{itemize}
  \begin{align*}
    \int_T \left( \sum_{j=1}^n F_2(\mathbf{q}_j) \cdot \phi_j \right) \cdot \nabla \phi_i\, dT 
    =
    \sum_{j=1}^n  F_2(\mathbf{q}_j) \cdot \left( \int_T \phi_j \cdot \nabla \phi_i\, dT \right) 
    % =:
    % \int_T \overline{F}_2 \left( \mathbf{q} \right)\cdot \nabla \phi_i \, dT 
  \end{align*}  
\end{frame}
...
\begin{frame}
  ...
  \begin{center}
    \begin{tikzpicture}
      \node at (0,0) {$\displaystyle SE_x^i = 
        \int_T \left| \left( 
            {F_2\left( \sum_{j=1}^n \mathbf{q}_j \cdot \phi_j \right)}
            -
            {\sum_{j=1}^n F_2\left( \mathbf{q}_j \right) \cdot \phi_j}
          \right) \cdot \nabla \phi_i \right| \,dT$};
      \draw[decorate, decoration=brace] (2.8,-.7)
      --node[below,yshift=-.075cm]{\scriptsize approximate} (0.4,-.7);
      \draw[decorate, decoration=brace] (-.1,-.7)
      --node[below,yshift=-.1cm]{\scriptsize exact} (-2.7,-.7);
    \end{tikzpicture}
  \end{center}
  \vspace{-.5cm}
  Evaluation of error term:
  \begin{itemize}
  \item Exact integration too expensive
  \item Maple's interactive functionality too restricted
    $\rightarrow$ Own tool
  \item We approximate integral by two dimensional Riemann sums
  \end{itemize}
  \begin{center}
    \includegraphics{dreiecke_weich_vs_prism.png}
  \end{center}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
  \item Motivation: $3n=3\cdot(o+1)(o+2)/2$ variables for basis functions of order $o$
  \item $SE_x^i$ can be interpreted as a function of $h_1,\dots,h_n$, $u_{x,1},\dots,u_{x,n}$, $u_{y,1},\dots,u_{y,n}$
  \item Intuitive approach: Fix all but one variable, and plot!
  \end{itemize}

  \begin{exampleblock}{Example}
    \begin{columns}
      \begin{column}{.45\textwidth}
    \includegraphics[scale=0.4]{{{ord2_varying_h1_default/y_10.0_10.0_10.0_10.0_10.0_0.0_0.0_0.0_0.0_0.0_0.0_0.0_0.0_0.0_0.0_0.0_0.0f00}}}
      \end{column}
      \begin{column}{.45\textwidth}
        \begin{eqnarray*}
          h_1 \text{ variable}, \\
...
\begin{frame}[allowframebreaks]
  ...
  \begin{itemize}
  \item Error in height component: 0
  \item If all height values, all $x$ momentums and all $y$ momentums are the same, error is 0 (can be shown mathematically).
  \item Exact analytical solution can have singularities 

    $\rightarrow$ but only for unreasonable combinations of heights
  \end{itemize}
  \framebreak
  \begin{itemize}
  \item If heights, $x$ and $y$ momentums do not differ very strongly, the error is relatively small.
  \item Error minimum (if one height value is varying) often at average value of the remaining height values.
  \end{itemize}
  \begin{center}
    \begin{tabular}{ccc}
      ...
      $h_2=h_3=10$ & $h_2=12, h_3=10$ & $h_2=12, h_3=8$
    \end{tabular}
  \end{center}
  \framebreak
  \begin{itemize}
  \item There was no observed difference in error terms for different water heights (i.e. only the height \emph{difference} was relevant).
  \end{itemize}
  \begin{center}
    \begin{tabular}{ccc}
    \includegraphics[scale=\zoomfactor]{{{slides_hrange_10/y_10.0_10.0_0.0_0.0_0.0_0.0_0.0_0.0f01}}} &
    ...
    $h_2=h_3=10$ & $h_2=h_3=100$ & $h_2=h_3=1000$
  \end{tabular}
...
\begin{frame}
  ...
  \begin{block}{Related questions}
    \begin{itemize}
    \item Different placement of support points (we used placement according to Dunavant, other possibilities include Gauss-Lobatto)
    \item Bathymetry (we used homogeneous SWE, i.e. bathymetry 0)
    \item Other norms
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}
  ...
  \end{eqnarray*}
  \begin{itemize}
  \item Sum approximation in the second line
...
\section{Approximate vs. ``exact'' result --- details}
\label{sec:appendix-approx-vs-exact-details}

\begin{frame}
  \frametitle{Stiffness term --- ``exact'' computation}
  \begin{eqnarray*}
    \int_T F_2(\mathbf{q}) \cdot \nabla \phi_i \, dT & = &
    \int_T
    \begin{pmatrix}
    ...
    \end{pmatrix}
    \cdot \nabla \phi_i \, dT \\
    & \stackrel{\mathbf{q}\approx\qapprox}{=} &
    \int_T
    \begin{pmatrix}
  ...
  \item We integrate over $x$ and $y$ (variables of $\phi_j$)
    ...
    $\rightarrow$ too expensive, not feasible
  \item Solution: Approximate function over the sum
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \end{eqnarray*}
  \begin{itemize}
  \item We show equations for $x$ momentum

    $\rightarrow$ analogous computation for $y$ momentum
  \item Sum approximation introduces error $\rightarrow$ how big?
  \end{itemize}
\end{frame}

\section{Using the matrices}
\label{sec:appendix-using-the-matrices}
...
\begin{frame}
  ...
  Time steps with distance $\tau$: $t_{k+1} = t_k + \tau$.
...
\part{From our previous presentation}

\begin{frame}
  \frametitle{From our previous presentation}
  \tableofcontents
\end{frame}
...
\begin{frame}
  ...
  \end{equation*}
  \begin{itemize}
  \item Complicated analytic solution (absolute value/rational function)
...
\begin{frame}
  ...
  \item Old-school: Numerical integration using the ``Riemann sum'' using $s$ stripes:
...
\begin{comment}
  ...
  First we set all support points to one single value ensuring homogeneous height and impulse. Then, we tackle each point individually and let it's values range over a certain domain.
  ...
  \begin{pmatrix}
    10 \\ 0
  \end{pmatrix}$ except $p_1$ which is set to $
  \begin{pmatrix}
    9 \\ 0
  \end{pmatrix}
