...
\begin{frame}
  ...
  \item Approximation of $\mathbf{u}$: 
...
\begin{frame}
  ...
  \begin{equation}
    ...
    \int_{\partial T} F(\mathbf{u}) \cdot n \phi_i \, ds = \sum_{e\in\left\{ e_1,e_2,e_3 \right\}} \int_{e} F^e \phi_i \, ds =: BI
  \end{equation}
  \begin{itemize}
...
\begin{equation}
  \int_{e} F^e \phi_i \, ds \approx \sum_{k=0}^{m} F^e(p_k) \underbrace{\phi_i(p_k) \cdot w_k}_{E_{ik}},
\end{equation}
...
\begin{frame}
  ...
  \todo{Sollten wir hier das Subscript $_j$ rausnehmen, um zu vereinfachen? Müsste sonst das Subskript nicht auch noch zu der Ableitung auf der linken Seite?}
  One equation for each component $j$ (i.e.\, for $h$, $u_x$, $u_y$):
  \begin{equation*}
  ...
  \begin{equation*}
    M \cdot \pd{
        \left( \mathbf{u}_1 , \dots , \mathbf{u}_n \right)
    }{t} = 
    \int_T F_j(\mathbf{u}) \cdot \nabla \phi_i \, dT 
    - \sum_{e\in\left\{ e_1,e_2,e_3 \right\}} \int_{e} F_j^e \phi_i \, ds
  \end{equation*}
...
\begin{frame}
  ...
  \todo{Obacht: $\mathbf{u}_i$ muss noch irgendwie anzeigen, dass es für den Zeitschritt $k+1$ gilt! Im Skript ausbessern und hier angleichen.Das stimmt hier noch gar nicht!}
