...
\begin{frame}
  ...
  \item \todo{Dreiecke disjoint Menge}
...
\begin{frame}
  ...
  \begin{itemize}
  \item Analytical solution for realistic scenarios still not found $\Rightarrow$ numerical approximation
  \end{itemize}
...
\begin{frame}
  ...
  \item Right triangle points: $\left(x_1,\begin{pmatrix}
      ...
      \end{pmatrix}\right), \dots , \left(x_n, \begin{pmatrix}
        h_n^R \\ u_n^R
      \end{pmatrix}\right)$
  \item Similar for left triangle
...
\begin{frame}
  ...
  \begin{center}
    $\Rightarrow$ Quality of approximation measured by $I := \int\limits_{x=0}^1 | N(x) - N'(x) |\ dx$ \todo{Hier wollte der Martin $L^1$-Norm drinstehen haben. Zu wenig Platz}
  \end{center}
\end{frame}
...
\begin{frame}
  ...
  \end{equation*}
  \begin{itemize}
  \item $N$ might have singularities (w.r.t. $x$ -- depending upon $h_i$ and $u_i$) 
  ...
  \item Maple couldn't find general analytic solutions \todo{Rausnehmen?}
...
\begin{frame}
  ...
  \begin{columns}
    \begin{column}{.55\textwidth}
      \begin{itemize}
      \item Analytic solution -- not feasible (zero crossings hard to compute)
      ...
      \item $\Rightarrow$ Old-school: 
        %Numerical integration using the 
        ``Streifenmethode'' using $s$ stripes
      \end{itemize}
    \end{column}
