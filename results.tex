...
In this work we focus mainly on the derivation of the shallow water equations\marginpar{Die habt ihr nicht hergeleitet, ihr baut darauf auf}, serving as a starting point for computation \marginpar{calculate ist nicht berechnen. Im Englischen ist das compute} of wave propagation, along with an analysis of the discretization problem, as well as numerical solutions for it.

\section{Adaptive triangle grid}
\label{sec:triangles}

For our purposes we use an adaptive triangle grid \marginpar{Wieso? Gutes Argument: Da gibt's das Sierpinski Framework, welches Dreiecksgitter verwendet. Deshalb machen wir auch mit Dreiecken weiter}.
...
\marginpar{Ich wuerde hier eine neue Section aufmachen, da das nichts mit Adaptiven Dreiecksgittern zu tun hat.}
...
The number depends on the degree of the test functions, as will be discussed in section \ref{sec:basis-functions-choice}. As it is impossible \marginpar{Da werden euch Mathematiker was anderes sagen :-). Impossible evtl. uncommon ersetzen} to calculate the propagation exactly due to its computational complexity, we need to find a numerical approximation.
...
\marginpar{Hier wurden noch keine Polynome eingefuehrt. Die fallen einfach vom Himmel.}
The higher polynomial degree we operate on, the more support points we will have. While this increases accuracy, it has a negative impact on efficiency\marginpar{Was ist bei euch Effizienz? Computation? Cache? Flops?}, so a proper balance is desired.

Another set of points are \emph{Gaussian quadrature} points, which we will use later on to approximate integrals along the edges\marginpar{Auch die Punkte im Dreieck selbst sind Gauss quadraturpunkte}.
...
For our project, we restricted ourselves to polynomial basis functions (depending on two variables $x$ and $y$) of variable degree $d$.
...
& & $4\,y-4\,xy-4\,{y}^{2}$ & $-4y$ & $4-4x-8y$ \\
    \bottomrule
  \end{tabular}
  \caption{Basis functions for low degree. While other basis functions can be derived by different means, we will focus on these throughout this project. The second column ($n$) denotes the number of basis functions used for the respective order.}
