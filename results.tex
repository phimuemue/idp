...
We have seen that the height error is zero in the scenarios under consideration, which we derived in section \ref{sec:height-error-p2}. Moreover, the error in momentum shows quite a non-predictable behavior.
...
%However, we chose not to plot the term directly, but instead apply the logarithm to it (since the resulting values can be extremely high). This is merely a measure to transform the values to make them easier to plot and does not affect the $x$ and $y$ scaling at all. I.e. we plot
...
\section{Order 1 --- Different orders of magnitude}
\label{sec:ord1-different-orders-magnitude}

We have seen so far that the more the heights resp. momentums differ, the bigger the error grows. We now research whether the \emph{difference} in heights resp. momentums is the \emph{only} thing that affects the error. I.e. we research what happens if the \emph{difference} of heights resp. momentums stays the same, but the order of magnitude is another.

\subsection{Standard values}
\label{sec:ord1-diff-ord-magnitude-std-values}

First we exemplary consider two cases using default values in two different orders of magnitude.
...
Please note that the \emph{difference} in heights resp. momentums is the same for both cases. However, the order of magnitude is another. We will restrict ourselves to errors for certain basis functions. The effects experienced here are similar for other basis functions.

\input{magnitude_comparison_default/tex}

Figure \ref{fig:magnitude_comparison_default} compares the two scenarios described above (for the $x$ momentum of the 2nd basis function --- similar effects can be seen for all basis functions and components). We see two plots that look identical. Thus, we might be tempted to conjecture that only the \emph{difference} of the heights and momentums affects the error.

\subsection{Differing height values}
...
We ensured again that the \emph{difference} in heights resp. momentums is the same for both cases. We show two resulting plots for the $x$ momentum of the second basis function in figure \ref{fig:magnitude_comparison_differing_heights}. Similar behaviour is seen for all other basis functions aswell. 
...
\subsection{Differing momentums}
\label{sec:ord1-magnitude-differing-momentums}

One thing remains to be done: We want to compare what happens if we not only change the height values, but also the momentums.
...
The resulting plots can be seen in figure \ref{fig:magnitude_comp_momentums}. Unexpectedly, we see different behaviour for the two plots. Even more surprising, we see that the error can be zero for the second case (where the height values are around 1000), and does not reach zero for the case for the first case (height values around 10).

\subsection{Differing heights and momentums}
...
Figure \ref{fig:magnitude_comp_heights_momentums} shows the error term of the $x$ momentum for the first basis function and the $x$ momentum for the second basis function. Surprisingly, these two scenarios yield different plots for the $x$ momentum of the first basis function. The error in the $x$ momentum for second basis function differ.

That shows that \emph{not only} the differences in height play a role for the error, but also the momentums. However, we still see that the errors do not differ too drastically for the different orders of magnitude.
