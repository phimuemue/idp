...
\todo{Hier wuerde ich noch kurz die Approximation beschreiben. Was passiert hier genau?}\done[Philipp]{Passt das dann so?}

Remember that we originally approximated as follows: $(h, u_x, u_y)^T \approx \sum_{i=1}^n (h_i, u_{x,i}, u_{y,i}) \cdot \phi_i(x,y)$ (see section \ref{sec:matrix-extraction}). While we approximate in the above equation as well, the transition from (\ref{eq:third-integral-second-line-1-analysis-part}) to (\ref{eq:third-integral-second-line-2-analysis-part}) uses a \emph{point-wise sampling approximation}, as it is computationally intensive for numerical simulations to determine the exact value for a continuous area if we used the original approximation (that would result in an integration of a rational function).
...
We do so by exemplary examining the third component and compute its ``exact value'' (i.e. we use our original approximatino introduced in section \ref{sec:matrix-extraction}):
...
We have now derived an ``exact solution'' (i.e. the solution that is obtained by our original approximation $\mathbf{q}=\sum_{i=1}^n \mathbf{q}_i\phi_i$ from section \ref{sec:matrix-extraction})\todo{Vorsicht!!! Ist das eine exakte Loesung? Was ist mit den Ableitungen? Wenn ihr diese mit Maple auswertet, solltet ihr das Beschreiben.}\remark{Ableitungen mit Maple? Ableitungen sollten alle exakt sein.} ($\widehat{F}_2(\mathbf{q}) \cdot \nabla \phi$ and $\widehat{F}_3(\mathbf{q}) \cdot \nabla \phi$) and one that uses an approximation for the rational fraction ($\overline{F}_2(\mathbf{q}) \cdot \nabla \phi$ and $\overline{F}_3(\mathbf{q}) \cdot \nabla \phi$ respectively). We now compare these two with the $L^1$ norm\todo{Warum nur L1? z. B. weil diese auch "andere" zum Vergleich nutzen}, as we did in the previous part. We chose the $L^1$ norm since it is a widely-used norm for comparing two functions with each other. Moreover, it can be naturally extended from the one dimensional case to the two dimensional case. For that we take the absolute value of the difference, and then integrate over the domain. We do this once for each component, hence we get these two terms called $SE_x$ and $SE_y$ respectively:
...
Figure \ref{fig:stiffness-analysis-log-singularities-ord-2} shows a plot for $\log_{10} h_1$. We observe that for order 2, the value $\log_{10} h_1$ ranges from 1.5 to about 5.5 within our triangle, which means that $10^{1.5} \leq h_1 \leq 10^{5.5}$. This means that $h_1$ (roughly) has to lie within the range $\left[ 31, 100000 \right]$ to provoke a singularity within the triangle --- very unlikely for small waves, but possible for shock fronts (as they \emph{might} occur e.g. in tsunamis). Thus, evaluating the integral may become inaccurate if we have a shock front.

\todo{very unlikely oder muesste das auch beruecksichtigt werden? \remark{ -- Gute Frage.}}\done{Passt das, was ich da geschrieben habe?}.
