...
The main problem \todo{Ist das ein Problem?} for computer simulation is the entire domain being discretized.
...
\section{Adaptive vs. non-adaptive triangle grid}
\label{sec:triangles}

For our purposes we could use an adaptive triangle grid. We do so, since a framework established at the TUM (called Sierpinski) is using this kind of triangle grid.
...
However, we research effects within a non-adaptive grid and focus on the respective equations.
\todo[Philipp]{Brauchts die Section? -- Wirkt etwas verloren.}

\section{Reference space and Discontinous Galerkin}
\label{sec:reference-space-dg}

Using a triangle grid, we can simplify a few steps in the calculation process.
...
\todo[Philipp]{Sollten wir diese section auf später verschieben? Sie sieht irgendwie so ``konkret'' aus im Vergleich zu den sie umgebenden Sections mit Galerkin/SWE.}
...
As mentioned before, we construct the basis functions with the help of some support points. However, arbitrarily choosing those can lead to the following disadvantages:

\begin{itemize}
\item As we will see later, we integrate over the basis functions (using Gaussian quadrature). Therefore it is convenient to choose support points in a way such that they can serve as support points in this Gaussian quadrature.
...
Ich wuerde auch zuvor die Quadratur auf dem Rand einfuehren und dann zur Matrix Formulierung uebergehen.\done{Die Gauss-Quadratur ist aber doch nur für das Randintegral nötig, die anderen Zeugs kann man doch auch ohne Gauss-Quadratur auf Matrixschreibweise bringen, oder?}
}

%To do that, we first have to get rid of \todo{"get rid of" ist zu salopp} the various terms containing $\mathbf{q}$.
Since $\mathbf{q}$ depends on the position, we cannot extract it from the integral. To simplify the terms further, we use the following approximation over all support points and corresponding basis functions for $\mathbf{q}$:
...
The values $\mathbf{q}_i$ are the $\mathbf{q}$-values at the corresponding support points. Since those do not depend on the position, it allows us to move them out of the integral, leaving integrals over various combinations of basis functions, which are all independent of $\mathbf{q}$, and hence can be precomputed.
...
\section{Matrix overview\todo{Passendere Ueberschrift finden}\done{Keine Ahnung, ob das jetzt besser ist}}
...
Since we are dealing with a one dimensional problem, we have to consider the flux function in one dimension, as well. Hence, we take the following as flux function, which is the same problem as started in section \ref{sec:shallow-water-equations} of the introduction, only mapped to one dimension\todo{Wie funktioniert das Mapping? Punktprodukt mit der Normalen --\remark{macht Punktprodukt mit der Normalen nicht die Trafo zwischen Spaces? Wir haben die Formel einfach so vom Martin bekommen.}}:
...
\begin{description}
\item[Support points according to Gaussian quadrature] \todo{Es gibt mehrer Versionen der Gauss quadratur. z. B. auch fuer $e^x$. Wir verwenden die Gauss Quadratur fuer Polynome und analysieren diese, da diese auch von anderen verwendet wird. Referenz nicht vergessen!} Since we eventually need to integrate over the edge (using Gaussian quadrature), it is convenient to choose the same support points to construct the polynomials.
to be applied, so we will choose our support points to coincide with them for easier computation. The corresponding results are summed up in section \ref{sec:results}.
...
There are several possibilities to overcome this problem. The first that might come to one's mind is to use a Gaussian quadrature (or any equivalent quadrature scheme for rational functions). However, even after extensive research we could not find any suitable quadrature method for rational functions.\done[Philipp]{Passt das so?}
...
\done{Auf Anraten vom Martin hab ich das Zeug mit den Gauss-Stützpunkten zur Herleitung der Kantenmatrizen gesteckt.}
...
We have seen in the plots that the errors in height and impulse show two very different behaviours. The impulse error highly depends upon the height values chosen, while the impulse values have a minor impact only. The height error, on the other hand, is largely depending on the chosen impulse values.

Moreover, and as a rule of thumb, we observe that the closer the height and impulse values of the support points are chosen, the smaller the error gets. This is good, since in practice it may be well the case that all support points have similar values and often do not show extreme deviations between each other.
...
Additionally, we saw that singularities do not play a significant role in our computations since they only occur for unrealistic scenarios. This is highly advantageous since we then can be sure that the integral exists and quadrature methods give meaningful results.
