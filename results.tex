...
The last scenario to examine is varying momentum values. We will use the same values as above, only with non-zero momentums.
...
Figure
\todo{Hier auch}
%\ref{fig:ord2_magnitude_comparison_default}
compares the two scenarios described above. There is no difference between the two, meaning that with one sole differing height value the error does not depend on the total magnitude.
...
\item In the second case, we leave $h_2=1002, h_3=7, h_4=1015, h_5=1011, h_6=999$ and $u_{x,i}=u_{y,i}=0$ for $i \in \{1,2,3,4,5,6\}$. The variable $h_1$ is the non-fixed variable. The range for $h_1$ will be $[1000-2, 1000+2]=[998,1002]$.
\end{itemize}
...
Figure
\todo{Hier auch}
%\ref{fig:ord2_magnitude_comparison_momentums}
compares the two scenarios. Again we cannot discern a difference between the two, which leads to the conclusion that with only differing height the error does not increase with magnitude.
...
\begin{itemize}
\item In the first case, we leave $h_2=12, h_3=7, h_4=15, h_5=11, h_6=9$ and $u_{x,i}=u_{y,i}=(-1)^i \cdot i$ for $i \in \{1,2,3,4,5,6\}$. The variable $h_1$ is the non-fixed variable. The range for $h_1$ will be $[10-2, 10+2]=[8,12]$.
\item In the second case, we leave $h_2=1002, h_3=7, h_4=1015, h_5=1011, h_6=999$ and $u_{x,i}=(-1)^i \cdot i$ for $i \in \{1,2,3,4,5,6\}$. The variable $h_1$ is the non-fixed variable. The range for $h_1$ will be $[1000-2, 1000+2]=[998,1002]$.
\end{itemize}
...
Figure
\todo{Hier auch}
%\ref{fig:magnitude_comp_heights_momentums}
shows something\todo{Das hier ausfuellen mit Plot-Daten}.
...
However, not only does it not increase strongly, situationally it can even be lower.
It depends on the actual values and setup of basis functions to determine the exact relation between different heights, making it inpractical to determine for the general case.
If that value is of interest, it will have to be examined with certain conditions predefined.

\todo{Ueberpruefen ob dieses Gelaber auch stimmt}

\remark{Vielleicht mathematisch zeigen, dass Null-Impuls nichts am Term aendert?}

\clearpage{}
\todo{clearpage, yay or nay?}
