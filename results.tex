...
As mentioned before, we construct the basis functions with the help of some support points. However, arbitrarily choosing those can lead to some disadvantages for the basis functions as described above. As the basis functions are derived from the support points by a matrix inversion, choosing support points cannot be done at random. If the points are in a certain pattern, it will result in a singular matrix and impossible to invert. There are several methods to pick the suppor points to avoid singular or otherwise malconditioned matrices, and we will choose the Gaussian quadrature points, as we will also use the same quadrature to integrate over the domain.

%we will will lead to a malconditioned matrix, which provides a malconditioned inverted matrix (if an inversion is even possible in that case), resulting in inaccurate basis functions
\todo{Diese Info haben wir vom Martin. Wie am besten referenzieren? Martin an alle: Ihr koennt z. B. schreiben, dass unterschiedliche Stuetzstellen existieren (Verweis auf Nodal DG methods). Diesbezueglich wurden die Gauss Integrationsstuetzstellen verwendet da aehnliche Vor- und Nachteile unter dem Aspekt der exakten Integration angenommen wurden.}. Since we will use Gaussian quadrature over these points, we will want support points that provide the best results for those. Thus, we choose support points such they coincide with support points for Gaussian quadrature.\done[Philipp]{Passt das?}\remark[Stjepan]{Ich weiss nicht, hoert sich noch immer etwas merkwuerdig an. Wieso gibt das die best results?}
...
%This was expected, since the impulse error is a rational function -- and thus not easy to imagine.
...
Note that $\int_T f(x,y)\,dT$ can be interpreted as the volume under the function $f(x,y)$, bounded by the borders given by our triangle $T$. This, however, means that we can approximate this volume by comprising it out of smaller parts. We decided to use prisms 
%\todo{Bezieht euch hierbei auf bereits angewandte Verfeinerungen in diesem Bereich. Eine fuer mich coole Sache ist z. B. die Triangulierung des DWD: www.dwd.de Diese verwenden fuer ihre Triangulierung auch die Verfeinerung wie ihr diese beschrieben habt.}\done{Sorry, keine Ahnung, was hier gemeint ist. Habe auch auf der Internetseite nichts passendes gefunden. Hab dann bissl was anderes als Alibi rein.}\remark[Stjepan]{Auch keinen Plan was gemeint ist, denke das passt so.} 
to approximate that volume.
...
%\todo{Ich halte es hier fuer unbedingt erforderlich, hervorzuheben, warum ihr keine fertigen Integrationsmoelichkeiten von Maple/python verwendet habt.}\done{Reicht das was jetzt kommt?}\remark[Stjepan]{Ich verstehe ehrlich gesagt nicht, wieso das so schlimm ist, hat weder lange gedauert, noch war es kompliziert. Haette wohl laenger gedauert uns in Numpy einzulesen deswegen... Aber wir koennten auch das folgende Argument nehmen:}

Similar to the situation from section \ref{sec:how-to-eval-I}, we considered several options to compute $SE$. However, we experienced similar problems evaluating the integrals exactly, which is why we chose a numerical integration. We decided to implement this method ourselves, since it was a simple implementation and was easy to integrate into our parser, which works with the provided equations before it pipes the result through to Gnuplot.%Unfortunately, we could not find any suitable Gaussian quadrature (or any equivalent numerical integration scheme) for this kind of functions, so we chose the technique described above.
...
\begin{figure}[ht]
  \centering
  \subfigure[Heat map for first order singularities. Plot shows $\log_{10} h_1 = \log_{10}\frac{20y}{-1+2y}$. White area is either undefined or not within the triangle $T$.]{
    \label{fig:stiffness-analysis-log-singularities-ord-1}
    \includegraphics[scale=0.5]{stiffness_analysis_log_sing_o1.pdf}
  }
  \hspace{.5cm}
  \subfigure[\todo{Plot erlaeuterungen sollten selbsterklaerend sein. Wenn ich nun nur auf diesen Plot sehe, koennte ich nicht ablesen, was dort abgebildet ist.}\done{KP.. das ist nicht gerade ein trivialer plot. Evtl. koennten wir gelb und rot vertauschen, oder waer das zu viel Arbeit? Rot ist ja normalerweise der "gefaehrliche" Bereicht, aber hier genau andersrum. Davon abgesehen kP wie wir das verstaendlicher machen koennen...} Heat map for second order singularities. Plot shows $\log_{10} h_1 = \log_{10} \frac{20\,y^2+\left(40\,x-30\right)\,y+20\,x^2-30\,x}{2\,y^2+
...
We can now -- for example -- set $x=y=0.25$ and obtain $  h_1 = \frac{1-20\cdot 0.25}{-1+2\cdot 0.25} = -8$. That means that for the point within the triangle located at $(0.25, 0.25)$ we would need a height of -8 to create a singularity. If we then take into account that $h_2=h_3=10$ we see that the value -8 strongly deviates from these values and can be considered very unlikely. Since it would be very tedious to try all values for $x$ and $y$ lying within our triangle, we can use a simple plot. However, we chose not to plot the term directly, but instead apply the logarithm to it (since the resulting values can be extremely high). This is merely a measure to transform the values to make them easier to plot and does not affect the $x$ and $y$ scaling at all. I.e. we plot
% \todo{Log scaling wird normalerweise nur kurz in der Beschreibung des Plots gegeben}\done{Mag sein, aber ich schätze, bei uns würde es das nur verkomplizieren, da bei uns weder $x$- noch $y$-Achse log-skaliert ist, sondern nur die $z$-Achse. Würde es daher lassen.}\remark[Stjepan]{Dito}
\begin{equation*}
\log_{10}\frac{20y}{-1+2y}.
\end{equation*}

We can see the graph for $\log_{10} h_1$ in figure \ref{fig:stiffness-analysis-log-singularities-ord-1}. We chose the coloring as follows:

\begin{itemize}
\item Green denotes values in $\left( -\infty, 0.7 \right] \cup \left[ 1.2, \infty \right)$. We colored these values green since they represent kind of the ``safe'' values. Remember that we are considering the logarithm (base 10), thus we can consider the interval $\left[ 10^{0.7}, 10^{1.2} \right] \approx \left[ 5, 15\right]$. These are values that may represent a non-shock wave.
\item Yellow denotes values in $\left[ 0.7, 1.2 \right]$ between 0.7 and 1.2. These represent dangerous values, since if we lie within this interval, we get a singularity even if we have non-shock waves.
  % \todo{Das entspricht nicht der Legende}\done{Eigentlich doch schon, oder?}\remark[Stjepan]{Auch dito}
\item White area is either undefined or out of the triangle.
\end{itemize}

As we can see, the value for $\log_{10}\frac{1-2x+24y}{-1+2y}$ is either 0 or below, or 1.25 or above (since there is only green in figure \ref{fig:stiffness-analysis-log-singularities-ord-1}, but no red at all). If we go back, we can conclude that either $h_1 \leq 10^{0.7} = 5$ or $h_1\geq 10^{1.2} \approx 15$. We can conclude that -- for the assignment $h_2=h_3=10$ -- the only values for $h_1$ that could provoke a singularity are strongly different from 10.
...
Figure \ref{fig:stiffness-analysis-log-singularities-ord-2} shows a plot for $\log_{10} h_1$. We observe that for order 2, the value $\log_{10} h_1$ ranges from 1.5 to about 5.5 within our triangle, which means that $10^{1.5} \leq h_1 \leq 10^{5.5}$. This means that $h_1$ (roughly) has to lie within the range $\left[ 31, 100000 \right]$ to provoke a singularity within the triangle, which is very unlikely for small waves, but possible for a sudden increase in height (which may be a valid consideration, for example in the case of shockwaves). Thus, evaluating the integral may become inaccurate under certain circumstances.

\todo{very unlikely oder muesste das auch beruecksichtigt werden? \remark{ -- Gute Frage.}}\done{Passt das, was ich da geschrieben habe?}\remark[Stjepan]{Passt, aber sollen wir uns noch darauf beziehen, dass das nicht an unserer Approximation liegt? Die Singularity taucht ja auch in der eigentlichen Gleichung auf.}\remark[Philipp]{Finde ich eine gute Idee.}
...
%\todo{Wg. der symmetrie? Dann solltet ihr das als Voraus-Haeppchen fuer den Leser hier nennen}\done{Ist aber nicht wg. Symm. glaube ich.}\done[Stjepan]{Agreed. Das liegt daran, das + kommutativ ist, und es egal is ob wir $h_2 \cdot bla$+$h_3 \cdot bla$ oder $h_3 \cdot bla$+$h_2 \cdot bla$ rechnen.}
...
%What one can guess\todo{anders, neutraler formulieren} from the things \todo{a thingy can also be a swimming pool} seen so far is that
...
%Unfortunately, we were not able to verify this result. The reason for that is that it does not hold true for all input values\todo{Ist das nicht ein Gegenbeweis?}.
