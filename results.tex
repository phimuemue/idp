...
\todo{Bei der Ueberschrift wuerde ich noch 'for SWE' anhaengen. Damit ist es aus meiner Sicht auch verstaendlicher, dass ihr die SWE spezifischen Formeln darstellt.}
...
\todo{encourage klingt so, wie wenn der Leser das noetig hat. Ausserdem sollten alle Symbole in didaktisch verstaendlicher Form bei der Einfuehrung beschrieben werden. 'cheat sheet' hat in einer wiss. Ausarbeitung auch nix verloren :-). Wie waere es mit "To give a better overview of the following mathematical symbols, we refer readers to appendix A"}

\section{Sum approximation vs. exact solution}
\label{sec:point-wise-appr-vs-exact-solution-intro}

While deriving the equations necessary for our calculations\todo{computations. Bitte auch ueberall calculation mit computation austauschen.}, we introduced several approximations. One of them was the approximation of the combination of various sums, as presented in the calculation of the stiffness matrix in section \ref{sec:stiffness-second-line}, in particular in the equations (\ref{eq:third-integral-second-line-1}) and (\ref{eq:third-integral-second-line-2}). For the second line (i.e. for the $u_x$ component), we had:
...
We will introduce this notation to refer to the approximated integrand\todo{Hier wuerde ich das nabla phi und auf de rrechten Seite die rechten Terme in den Klammern weglassen}:
...
While we approximate here as well, it is the point-wise sampling approximation and is used in both cases, as it is impossible \todo{Besser evtl.: computational intensive for numerical simulations} to determine the exact value for a continuous area.
...
Similarily to before\todo{the previous steps}, we can introduce a hat notation to signal the summation of the individual components, only this time we define it for the entire integrand. Doing that, we can simply the above term\todo{In this way, we can simplify the...}:
...
\todo{Als Ueberschrift vllt. einfach "Meaning of $SE$ schreiben um keine Fragezeichen in Ueberschriften zu verwenden.}
...
If one considers the terms $SE_x$ and $SE_y$, it is clear that these terms -- similar to the term $I$ obtained in section \ref{sec:how-to-eval-I} -- contain rational fractional functions and are likely quite hard \todo{computational intensive} to compute exactly, maybe even impossible.
\todo{It was infeasible to compute those integrals using analytical software such as maple.}

While integrating in the one-dimensional case can be easily\todo{Kein easily, da das die vorherige Arbeit schlechtredet!} approximated by the rectangle method (which can be made arbitrarily precise by a varying number of rectangles\todo{Das wuerde ich entfernen, da es nicht allgemeingueltig ist}), the two-dimensional case (i.e. integrating something like $\int_{0}^1 \int_0^{1-x} f(x,y)\, dy\, dx$) requires a slightly more sophisticated approach. Instead of lines we now need to divide the area in smaller areas to sample, but at the same time make sure not to exceed the bounds. Since we are dealing with a triangle, we cannot use rectangle shapes for that.
...
Note that $\int_T f(x,y)\,dT$ can be interpreted as the volume under the function $f(x,y)$, bounded by the borders given by our triangle $T$. This, however, means that we can try\todo{Kein try} to approximate this volume by comprising it out of smaller parts. We decided to use prisms \todo{Bezieht euch hierbei auf bereits angewandte Verfeinerungen in diesem Bereich. Eine fuer mich coole Sache ist z. B. die Triangulierung des DWD: www.dwd.de Diese verwenden fuer ihre Triangulierung auch die Verfeinerung wie ihr diese beschrieben habt.} to approximate that volume.

In principle, we devide the triangle in a way such that we cut the cathetuses\todo{cathetii wenn ich mich recht erinnere oder triangle legs} (of length 1) into smaller pieces of length $\frac{1}{L}$ (the integer $L$ denotes the parameter for our integral approximation\todo{Auf Klammern verzichten, am besten als Satz ausschreiben}). Then, we can generate the centers of the resulting triangles as follows. First, we consider a set of \emph{all possible} centers:
...
We can now sample the height at the centers and compute the appropriate volume of the corresponding prism. Therefore, we have to multiply the height by the base area of the respective triangle. The area of one triangle is $\frac{1}{2 L^2}$. This results then in the following equation\todo{, resulting in the following equation}:
...
The factor $\frac{1}{2}$ is needed since we actually approximate the volume denoted by the integral by the volume of a prism\todo{with the same base area as our .... "Im Deutschen wird oft 'dass' verwendet. Im Englischen von dem her, was ich von bisherigen Papern gelesen habe, sehr selten"}, which has the same base area as our triangle $T$ ($\frac{1}{2}$).
...
\begin{figure}[ht]
  ...
  \hspace{.5cm}
  \subfigure[\todo{Plot erlaeuterungen sollten selbsterklaerend sein. Wenn ich nun nur auf diesen Plot sehe, koennte ich nicht ablesen, was dort abgebildet ist.} Heatmap for second order singularities. Plot shows $\log_{10} h_1 = \log_{10} \frac{20\,y^2+\left(40\,x-30\right)\,y+20\,x^2-30\,x}{2\,y^2+
...
Let us\todo{Let's fetz sollte in Ausarbeitungen vermieden werden. Besser waere: Next, we consider order 1 by setting..., oder so aehnlich.} start out ba trying order 1, and setting $h_2=h_3=10$. We then obtain the following (by substituting $\phi_1$ to $\phi_3$ as defined in table \ref{tab:basis-functions-for-low-degrees}):

\begin{equation*}
  h_1 = \frac{20y}{-1+2y}
\end{equation*}

We can now -- for example -- ``try''\todo{kein try :-)} $x=y=0.25$ and obtain $  h_1 = \frac{1-20\cdot 0.25}{-1+2\cdot 0.25} = -8$. That means:\todo{E.g. this leads to...} For the point within the triangle located at $(0.25, 0.25)$ we would need a height of -8 to ``generate'' a singularity. If we then take into account that $h_2=h_3=10$ we see that the value -8 \emph{strongly} deviates from these values and can be considered very unlikely. Since it would be very tedious to try all values for $x$ and $y$ lying within our triangle, we can use a simple plot. However, we chose not to plot the term directly, but instead apply the logarithm to it (since the resulting values might be huge). I.e. we plot\todo{Log scaling wird normalerweise nur kurz in der Beschreibung des Plots gegeben}
\begin{equation*}
...
We can see the ``graph'' (more precisely a heatmap\todo{auch das wird normalerweise in der plot beschreibung erwaehnt}) for $\log_{10} h_1$ in figure \ref{fig:stiffness-analysis-log-singularities-ord-1}. We chose the coloring as follows: Everything that is red resp. green denotes values above 1.2 resp. below 0.7. Yellow denotes values between 0.7 and 1.2\todo{Das entspricht nicht der Legende}. White area is either undefined or out of the triangle.
...
Following are reports of various scenarios we have tested, trying to find out any common behavior between the values. All the following graphs will show the calculated error in the stiffness terms for both components, i.e. $SE^i = \left(SE_x^i, SE_y^i\right)$ for $i=1,\dots, n$.
...
\todo{Regarding to section...} As was mentioned in section \ref{sec:basis-functions-choice}, choosing order 1 requires 3 basis functions $\phi_1,\dots,\phi_n$. These depend on the three components (height, $x$ momentum, $y$ momentum) of the three support points. First, we will look into the impact the value $h_1$ (i.e. the height of the first support point) has on the error terms. We fix\todo{Ich wuerde "keep the following values constant" besser finden} the following values:
...
While this is a good result for us\todo{in this circumstance}, the more interesting plots in figure \ref{fig:stiffness-analysis-order-1-standard-values-h1} are the remaining ones, which all seem to be the same. We can see that the error is minimal for $h_1=10$ and grows as $h_1$ deviates from this value (apparently the same in both directions). \todo{The shape is similar to a...} It looks like a parabola, which makes sense when we examine the equations for the first order.
...
Moreover, it is again noteworthy that the minimum error occurs for $u_{x,1}=0$ since that\todo{Evtl. 'this', bin mir aber nicht sicher} equals the values of $u_{x,2}$ and $u_{x,3}$.
...
The results can be seen in figure \ref{fig:stiffness-analysis-order-1-standard-values-uy1}. As one could have suspected\todo{Keine Spekulationen/Verdaechtigungen, nur harte Fakten.}, the plots for varying $u_{y,1}$ look like these for $u_{x,1}$, except that when varying $u_{y,1}$, the errors in the $y$ momentum of basis function 1 and 2 have the parabola-like shape. Similar to the situation before, the error is minimal for $u_{y,1}=0$.
...
\todo{We focus on relevant aspects, such as...}
What is interesting here is that \emph{all} error plots have characteristic shapes (i.e. none of them is 0, like in previous examples). Moreover, it seems that the error is minimal if $u_{x,1}=u_{y,1}=0$. Once again one can see the similarities between some plots, especially with regard to mirroring/rotating. We will later omit plots that are the same or mirrored.
...
Up until now we have only considered scenarios in which all fixed heights and momentums were equal. We will now focus on \todo{choosing different fixes values, ...} what happens if we choose different fixed values, for example $h_2=10$ and $h_3=11$.
...
Let us\todo{Kein let's fetz} see what it looks like if we now set $h_2=12$ and $h_1=10$.

\input{ord1_differing_h2_h3_12_10/tex}

The resulting plots are shown in figure \ref{fig:stiffneses-analysis-ord1-h2-12-h3-10} and they look as expected\todo{Wenn das so erwartet wurde, warum zeigt ihr diese dann?}: The minimum error is again at $h_1\approx 11$. That means, we can conjecture that it does not matter which of the other heights does differ, but only the difference between the two. This makes sense when examining the equations, which we will cover later\todo{Wg. der symmetrie? Dann solltet ihr das als Voraus-Haeppchen fuer den Leser hier nennen}.
...
What one can guess\todo{anders, neutraler formulieren} from the things \todo{a thingy can also be a swimming pool} seen so far is that -- if we leave all $u_{x,i}=u_{y,i}=0$ ($i=1\dots n$) and take $h_1$ as axis variable -- the error seems to be minimal if $h_1\approx \frac{h_2+h_3}{2}$.
...
Unfortunately, we were not able to verify this result. The reason for that is that it does not hold true for all input values\todo{Ist das nicht ein Gegenbeweis?}. To illustrate this, let us consider an (admittedly extreme) example where we choose $h_2=16$ and $h_3=4$.
...
One can see there\todo{Kein 'one'} that the error for all $y$ momentums is zero. Moreover, the error is zero for the $x$ momentum of the 5th basis function. The error becomes zero at $u_{x,1}=0$, i.e. if $u_{x,1}$ is the same as all other momentums.
...
\todo{Was sind non-standard values?}We will now alter some values and see how this affects our error plots. We will do similar alterations to the ones used in section \ref{sec:stiffness-analysis-ord1-non-std-values} and see if we have a similar behavior to before.
...
As expected from our calculations\todo{calculus?} in section \ref{sec:stiffness-analysis-ord2-default-var-ux1} and \ref{sec:basis-functions-other-properties}, the errors for the $y$ momentum of the 3rd basis function and the $x$ momentum of the 5th basis function is still 0. For a detailed calculation\todo{calculus?}, refer to section \ref{sec:stiffness-analysis-ord2-default-var-ux1}.
...
We now set all heights $h_2$ to $h_6$ to the value 11. We do so, because we hope \todo{Hoffen kann man in der Kriche. Hier koennte man z. B. "to examine..." schreiben} to see that we now again obtain parabola-like shapes (as in the case where all heights were set to 10). The results are shown in figure \ref{fig:ord2_varying_h1_h2-h6_11} and confirm our expectations: We have parabola-like shapes, whose minima are without exception at $h_1=11$.
...
Now we have the first time where some plots look drastically\todo{evtl. strongly} different than before.
...
Let us now inspect what happens \todo{what happens kommt sehr oft vor. Besser verhindern und anders schreiben.} if we do not change the $x$ momentum but the $y$ momentum. As a showcase example, we will use $u_{y,2}=1$ and leave all other momentums at 0 (-- all heights are still 10).

The plots are shown in figure \ref{fig:ord2_varying_ux1_uy2_1} and are likely to surprise \todo{Ueberaschungseier gibt es im Supermarkt :-). 'are likely to show important features' waere evtl. eine bessere Wahl} us. Because these plots are likely to surprise us, we split them up into the errors for the $x$ momentums and the errors for the $y$ momentums.
...
We will now come back to only modifying the $x$ momentums. We will set them all to 1 and leave $u_{x,1}$ as axis variable. We expect \todo{Was ihr erwartet, spielt keine Rolle sondern nur harte Fakten. Wenn man Sachen erwartet stellt sich oft die Frage, warum man das ueberhaupt getestet hat wenn man das Ergebnis schon erwartet hat.} parabolae with minima at $u_{x,1}=1$ (since all other $x$ momentums are 1). We can see the plots in figure \ref{fig:ord2_varying_ux1_ux2-ux6_1}.
...
If we have a situation, where all heights are the same, all $x$ momentums are the same and all $y$ momentums are the same, the error becomes 0 for both components. For the respective calculations, refer to the previous sections.
...
The sierpi framework uses height values around 10 and momentums whose absolute value might grow up to 4\todo{Aeh, nein. Auf freiem Meer sind es auch mal 5000 und mehr Meter :-). Ich wuerde auch den folgenden Satz einfach weglassen.}. The difference in height is almost always very small (say less han 0.5), while the difference in momentums might grow up to 1 --- even if that is not too often the case.
