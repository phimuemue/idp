...
The essence of the equations is based on the conservation law (with regards to mass and impulse conservation) as presented in the general continuity equation.
...
With the bathymetry at zero, this leads to:
...
Since we will need to discretize the space eventually, we will have to work on a disjunct set of polygons, together incorporating the entire simulation space.
...
Since it is a boundary integral of a triangles, we can split it up into several integrals, one for each edge.
This also implies a normal vector $\mathbf{n}_e$ per edge.
...
Our reference space assumes a leg of length 1 and consequently the hypothenuse of length $\sqrt{2}$.
...
Each triangle has several integration points which are interpolated to obtain a polynomial for the corresponding triangle.

Figure \ref{fig:two-triangles-and-some-support-points} shows two adjacent triangles and exemplary positions of the integration points.
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[scale=2]
    ...
    \end{scope}
  \end{tikzpicture}
  \caption{Two adjacent triangles with integration points}
...
We have seen that the height error is zero in the scenarios under consideration. Moreover, the error in momentum shows quite a non-predictable behaviour.
%This was expected, since the momentum error is a rational function -- and thus not easy to imagine.
It can vary strongly, both in shape and in scale, when adjusting the height values, while the momentum values have a minor impact only.
...
Let us shortly contrast the two different approximation methods. Equations \eqref{eq:contrast-exact-solution} and \eqref{eq:contrast-approx-solution} show the exact and the approximative solution, respectively.
...
Moreover, our experiments have shown that there are situations in wich the approximate solution is equal to the exact solution. Moreover we saw that it is quite hard to predict how
