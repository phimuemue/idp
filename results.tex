...
Since we are visualizing norms of (i.e. integrals over) absolute error terms, it is difficult to tell wich results are ``good'' and ``bad''. But we can see which results are better than others, and -- of course -- generally smaller values are better then big ones. That is, we can compare two errors and determine the better of both, but -- due to the nature of the integral -- we can not tell \emph{by how much} one is better than the other.
...
We have seen that the height error is zero in the scenarios under consideration, which we derived in section \ref{sec:height-error-p2}. Moreover, the error in momentum shows quite a non-predictable behaviour.
