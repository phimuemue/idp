...
\todo{Hier wuerde ich noch kurz die Approximation beschreiben. Was passiert hier genau?}\done[Philipp]{Passt das dann so?}\remark[Stjepan]{Hab das hier geaendert, da stand point-wise approximation fuer die obige Gleichung da, was ja nicht stimmt. Die point-wise approximation war die Summe ueber die einzelnen Punkte.}
...
We do so by exemplary examining the third component and compute its exact value (i.e. we use our original approximation introduced in section \ref{sec:matrix-extraction}):

\remark[Stjepan]{Wieso third? Second wuerde mehr Sinn machen imo}
\begin{eqnarray*}
  \int_T F_3\left(\mathbf{q}\right) \cdot \nabla \phi \, dT & = &
  \int_T
  \begin{pmatrix}
...
We can now -- for example -- set $x=y=0.25$ and obtain $  h_1 = \frac{1-20\cdot 0.25}{-1+2\cdot 0.25} = -8$. That means that for the point within the triangle located at $(0.25, 0.25)$ we would need a height of -8 to create a singularity. If we then take into account that $h_2=h_3=10$ we see that the value -8 strongly deviates from these values and can be considered very unlikely. Since it would be very tedious to try all values for $x$ and $y$ lying within our triangle, we can use a simple plot. However, we chose not to plot the term directly, but instead apply the logarithm to it (since the resulting values can be extremely high). This is merely a measure to transform the values to make them easier to plot and does not affect the $x$ and $y$ scaling at all. I.e. we plot\todo{Log scaling wird normalerweise nur kurz in der Beschreibung des Plots gegeben}\done{Mag sein, aber ich schätze, bei uns würde es das nur verkomplizieren, da bei uns weder $x$- noch $y$-Achse log-skaliert ist, sondern nur die $z$-Achse. Würde es daher lassen.}\remark[Stjepan]{Dito}
\begin{equation*}
...
\todo{very unlikely oder muesste das auch beruecksichtigt werden? \remark{ -- Gute Frage.}}\done{Passt das, was ich da geschrieben habe?}\remark[Stjepan]{Passt, aber sollen wir uns noch darauf beziehen, dass das nicht an unserer Approximation liegt? Die Singularity taucht ja auch in der eigentlichen Gleichung auf.}
