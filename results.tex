...
exact and approximate solution of the Lax-\-Friedrichs-\-Flux}
...
\begin{description}
\item[Exact solution] We supply the polynomials $p^L$ and $p^R$ (depending on $x$) into the Lax-Friedrichs-Flux, and compute its exact value. The resulting values are rational functions depending on $x$. We call these functions $N\left(x\right)$ (a vector which has a rational function for the height as its first component, while the second component represents the momentum).
...
\todo{Explain why we do it with equidistant support points}
...
\subsection{Thre points -- one point variation}
...
Moreover, and as a rule of thumb, we observe that the closer the height and impulse values of the support points are chosen, the smaller the error gets. This is good, since in practice it may be well the case that all support points have similar values and often do not show extreme deviations between each other. In practice, one will choose the triangle small enough that the variations within a triangle are not too large. For these triangles, the approximation yields good results.
