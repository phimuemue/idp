...
\item Yellow denotes values in $\left[ 0.7, 1.2 \right]$ between 0.7 and 1.2.
...
As we can see, the value for $\log_{10}\frac{1-2x+24y}{-1+2y}$ is either 0 or below, or 1.25 or above (since there is only red in figure \ref{fig:stiffness-analysis-log-singularities-ord-1}, but no yellow at all). If we go back, we can conclude that either $h_1 \leq 10^{0.7} = 5$ or $h_1\geq 10^{1.2} \approx 15$. We can conclude that -- for the assignment $h_2=h_3=10$ -- the only values for $h_1$ that could provoke a singularity are strongly different from 10.
