...
In this work we focus mainly on computations based on the shallow water equations\marginpar{Die habt ihr nicht hergeleitet, ihr baut darauf auf\done[Philipp]{Passt das so?}}, serving as a starting point for computation \marginpar{calculate ist nicht berechnen. Im Englischen ist das compute} of wave propagation, along with an analysis of the discretization problem, as well as numerical solutions for it.

\section{Adaptive triangle grid}
\label{sec:triangles}

For our purposes we use an adaptive triangle grid\marginpar{Wieso? Gutes Argument: Da gibt's das Sierpinski Framework, welches Dreiecksgitter verwendet. Deshalb machen wir auch mit Dreiecken weiter\done[Philipp]{Passt das?}}. We do so, since a framework established at the TUM (called Sierpinski) using this kind of triangle grid.
...
\section{Reference space and DG}
\label{sec:reference-space-dg}

\marginpar{Ich wuerde hier eine neue Section aufmachen, da das nichts mit Adaptiven Dreiecksgittern zu tun hat.\done[Philipp]{Ok.}}

Using this scheme\marginpar{Welches scheme?}, we can simplify a few steps in the calculation process.
...
The domain support of the entire triangle is given by $T$ (the set of all points on the triangle)\marginpar{Meint ihr damit das gesamte Dreieck? oder die Quadraturpunkte?}, and the set of edges $E$, enumerated with $e_i$, $\forall i \in {1,2,3}$.
...
The points in the triangle are support points.
The number depends on the degree of the test functions, as will be discussed in section \ref{sec:basis-functions-choice}. As it is uncommon to calculate the propagation exactly due to its computational complexity, we need to find a numerical approximation.
...
In our case, the test functions will be polynomials of a certain degree.\todo[Philipp]{Polynome eingeführt. Passts so? Sind test functions das gleiche wie basis functions?}

The higher polynomial degree we operate on, the more support points we will have. While this increases accuracy, it also makes the involved equations more complex and complicates (or even disallows) certain simplifications\done[Philipp]{``Efficiency'' is raus.},
%as a negative impact on efficiency\marginpar{Was ist bei euch Effizienz? Computation? Cache? Flops?}, 
so a proper balance is desired.

Another set of points are \emph{Gaussian quadrature} points, which we will use later on to approximate integrals along the edges\marginpar{Auch die Punkte im Dreieck selbst sind Gauss quadraturpunkte \todo[Philipp]{Das glaube ich nicht, Timm!}}.
...
The (minimum) number of basis functions $n$ is not arbitrary, but it depends on the degree of the polynomials. If we only employ polynomials of degree 0 (i.e.\,constants), we only need one basis function. Increasing the degree by 1 involves introducing variables $x$ and $y$\marginpar{X und Y fallen hier vom Himmel. Fuehrt diese evtl. als $p_i = (x_i, y_i)^T$ ein.}. Thus, we need three polynomials to construct every polynomial of degree 1, one to create polynomials containing $x$, one to create polynomials containing $y$ and one to create other constant polynomials.

Generalizing this scheme we observe the following: if we want to use polynomials of degree $d$, a polynomial can contain terms $x^a y^b$ with $a+b \leq d$. A simple combinatorial arguments indicates that we need $\frac{(d+2) \cdot (d+1)}{2}$ basis functions to span the function space containing all the polynomials up to degree $d$ \marginpar{Hier fehlt die Referenz}.
...
However, arbitrary choosing those can lead to accuracy loss in the best case, and malconditioned matrices, which can prevent precomputing arrays in the worst\marginpar{Der Satz klingt seltsam, insbesondere das Ende. Wenn ihr solche Statements macht, muesst ihr immer Referenzen angeben.}.
...
For higher degree, we choose the points derived by Dunavant (see \cite{dunavant1985high})\marginpar{Gebt diese Referenz auch bei der Tabelle an}. The distributions of support points are illustrated in figure \ref{fig:triangle-support-points-var-degrees}.
...
& & $4\,y-4\,xy-4\,{y}^{2}$ & $-4y$ & $4-4x-8y$ \\
    \bottomrule
  \end{tabular}
  \caption{Basis functions for low degree. While other basis functions can be derived by different means, we will focus on these throughout this project. The second column ($n$) denotes the number of basis functions used for the respective order. See \cite{dunavant1985high} for more information.\todo[Philipp]{Stimmt das wirklich?}}
...
As you \marginpar{Niemals den Leser per 'you' ansprechen. Besser: As we can see in table ...} can see in table \ref{tab:basis-functions-for-low-degrees}, there are some symmetries for the basis functions. We will point out some of these symmetries here because they will help us in the analysis later.
...
It must be the same because of the following: The sum of all basis functions $\phi_i$ yields a new polynomial, whose coefficients must be all zero, except the ``constant coefficient'' that must be 1. This can be seen as follows: Imagine you would like to solve the following system of linear equations:
...
If you solve the above system, you arrive at the fact that $\alpha_{0,0}=1$ and all other $\alpha_{i,j}=0$ --- at least if all columns within the huge matrix are linearly independent (which is the case if all support points are pairwise different).
...
For time stepping, we use the classical Euler method. It is a well-known fact that there are other techniques such as Runge-Kutta, but since this subproblem does not affect our later analysis, we use it to demonstrate the technique of time stepping (see \cite{hans2009schwarz} for more information).
...
Lowering it results in more accurate values, but reduces the stability of the algorithm. This relationship is discussed in detail in \cite{cockburn1999discontinuous}.
...
There are several possibilities to overcome this problem. The first that might come to one's mind is to use a Gaussian quadrature. However, using a Gaussian quadrature requires that the integrand at least ``looks somehow'' like a polynomial -- something we explicitly can \emph{not} be sure of.
...
From equations (\ref{eq:1}) and (\ref{eq:2}) you can see the reason that you can choose the $y$ momentums as you like it --- it won't influence the error term $SE_x^i$.
...
% \subsection{When the error is zero}
