...
One of those points is the discretization of the continuous space into finite segments.%, usually triangles.
%Another is the simulation of the domain of each segment by sampling specific points within that segment.
...
The main problem \marginpar{Ist das ein Problem?} for computer simulation is the entire domain being discretized.
...
With a suitable library for coordinate projection, we can rotate triangles and edges into a space that's relevant for us.
...
%This will help with further calculations, because some of them will rely on the length of an edge, which can be conveniently omitted (or set to the constant $\sqrt{2}$ respectively).
...
Generalizing this scheme we observe the following: if we want to use polynomials of degree $d$, a polynomial can contain terms $x^a y^b$ with $a+b \leq d$. A simple combinatorial argument indicates that we need $\frac{(d+2) \cdot (d+1)}{2}$ basis functions to span the function space containing all the polynomials up to degree $d$ \marginpar{Hier fehlt die Referenz}.
...
A diagonal matrix can be derived by using \emph{orthogonal} polynomials\todo{Referenziert hier am Besten die Jacobi Polynome.}.
For polynomial degree 1, it is no problem to obtain orthogonal polynomials using the technique described in section \ref{sec:basis-functions-choice}\todo{Satz hat wenig Aussage.}.
...
When analyzing wave propagation along a grid of triangles, it's\todo{Abkuerzungen sind schlechter Stil. Immer ausschreiben. Bitte auch ganzen Text durchsehen} essential to know how much of each quantity is passed from one triangle to another. This value is called the flux and it's determined by the so-called flux function. The quantities that interest us are the height ($h$, proportional to the volume and hence mass), and momentum ($u$, proportional to velocity $q$), where the momentum is a vector for every spatial dimension\todo{Ihr beschreibt hier altbekanntes wieder. Evtl. weglassen?}. The latter, however, can be remodeled using a one-dimensional approach by parametrizing the relevant coordinates.

This may sometimes be acquired by exact means, however those computations are long and complex and can result in prohibitively high computation times. On the other hand, approximations through various numerical methods can speed this process up significantly. We want\todo{Wir wollen das nicht analysieren, wir \textbf{haben} das im Rahmen dieser Arbeit analysiert und evaluiert :-)} to analyze what kind of accuracy loss that implies and if it's a suitable method for realistic computation.
...
Depending on simulation circumstances, $\alpha$ is set globally and treated the same for all triangles or is determined based on the quantities stored for certain triangles at run time, which obviously results in higher computation time\todo{Nein: Adaptive $\alpha$ bestimmung ermoeglichst genauere und voraussichtlich groessere Zeitschrittweiten und damit voraussichtlich kuerzere Simulationen}. The various methods and their properties are discussed in \cite{leveque2002finite}.
...
Depending on how $\alpha$ is handled, it results in different methods. Sometimes it's set globally and treated the same for all triangles, other times it's determined based on the qualities of certain triangles at run time, which obviously results in higher computation time. The various methods and their properties are discussed in \cite{leveque2002finite}. For our purposes, we will leave it invariant in time and space across the whole domain.
...
Since we are interested in the situation along the adjacent edges, which is a linear curve, we can remodel the problem to one dimension, which will reduce the computational complexity. This means that we do not actually construct two-variable polynomials for the whole triangles\todo{Braucht man auch nicht, wenn man die 2 Variablen mit einer enizigen parametrisieren wuerde}, but instead consider just polynomials that depend on one variable $x \in [0,1]$. The variable $x$ simultaneously traverses both adjacent edges.\todo{Beruft euch z. B. darauf, dass die Berechnung im Kanten Referenzraum identisch ist. Vorausgesetzt, das wurde vorher erklaert :-).}
...
We do so for the left and the right triangle, and for the height and the momentum. We call the points we've \todo{Keine Abkuerzungen} chosen the \emph{support points} for the left and right triangle. For convenience, we have decided to use a naming convention for these points.
...
Our goal will then be to compare $N\left(x\right)$ against\todo{Evtl. 'with'? Bin mir nicht sicher} $N^\prime\left(x\right)$ (component-wise, of course). We compare the quality of an approximation by considering the following term:

\begin{equation}
  \label{eq:integral-norm-definition}
  I := \int\limits_{x=0}^1 | N\left(x\right) - N^\prime\left(x\right) |\  dx,
\end{equation}
where the integration is to be understood component-wise\todo{Das Kind (die Norm) hat auch einen Namen: L1 Norm wenn mich nicht alles taeuscht}.
...
The main problem \todo{challenge, nicht problem} with exact integration is that Maple has to compute the zeroes of $N\left(x\right)-N^\prime\left(x\right)$, which is, depending on the degree of the polynomials, a time consuming task.
...
\todo{Ich wuerde bei jedem bullet-point noch fett ein schlagwort davor schreiben}
\begin{itemize}
\item $N\left(x\right)$ and $N^\prime\left(x\right)$ intersect when supplied the support points, i.e.\,
  ...
  Knowing this, we can derive some zeroes (but possibly not all)\todo{Wo kommen denn auf einmal die 'zeros' her? Meinst du die zero-crossings?}. If we compute
  \begin{equation*}
...
\item Another approach was to use Maple's numerical integration facilities. If we tell Maple to
  ...
  \todo{Stimmt zwar, wuerde ich aber etwas anders schreiben: Computational infeasible for detailed analysis of higher order...}
  \todo{Ich wuerden das ganz nach vorne ziehen. Erst habt ihr es naemlich mit Maple versucht. Das hat zu lange gedauert. Dann ging's weiter zur zweiten methode}
...
We eventually \todo{Eventuell?} went for the last of these approaches\todo{Hier schreibt ihr schon wie es weiter geht, erst in den folgenden Saetzen aber, warum ihr das gemacht habt. Ich wuerde die Entscheidung nach hinten setzen}. It is just as well possible to use the second one\todo{Wenn ihr erster/zweiter/dritter verwendet, verwendet auch enumerate statt itemize}, or the first, although this requires to tell Maple that it should \emph{first} make all necessary substitutions, and \emph{second} plot the resulting graphs. This is not only harder to accomplish but also results in a mess of code, but would not produce more accurate results, which is why we did not explore this option further.
...
We tried these calculations for other scenarios, including a variation in support points\todo{... leading to similar results. [Den folgenden Satz wuerde ich dann streichen]}.
The results were always the same, that the height would have to differ in absurd levels to even approach the required region of the singularity.
...
\subsection{Some considerations to take into account\todo{Schoener Section Name}}
...
This implies that $I$ (see equation (\ref{eq:integral-norm-definition})) also depends on the single points\todo{Ich wuerde die Vektoren quergestellt und transponiert schreiben. Dann sieht das nicht so zerhackt aus}
$
\begin{pmatrix}
  h_1^L \\ u_1^L
\end{pmatrix},\dots,
...
However, we realized that this method had some downsides. We are always interested in plotting two graphs, one for the height error and one for the momentum error (both components of $I$). Showing two plots simultaneously affected by the same sliders proved to be a challenge, and was not doable with the native plotting services due to limited customization. It may have been able to write a custom Maplet (interactive Maple applet) which did what we wanted, but that turned out to be just as cumbersome due to Maple's limited applet functionality.\todo{Ihr koenntet das "wissenschaftlicher" formulieren ala "Due to restrictions of Maples functionality regarding applet programming, our requirements were not satisfied". Oder so.}
...
We decided to develop a tool that helps us visualizing our data properly.
We implemented a simple\todo{Simple muss raus :-)} program that is capable of doing the following:
...
The left part of subfigure \ref{subfig:fixing-p2-in-first-example} shows the (normalized\todo{Was ist hier normalisiert? Was ist die 'baseline'?}) error in the $h$ component, while the right half depicts the error in the $u$ component.
...
We did the same thing for three support points along each edge (and it can be done for an arbitrary number of support points). You\todo{You} can see the results in figure \ref{fig:three-points-equal}. In general it seems that the structure of the error plots is quite similar across different settings: the error in the height component is almost zero, while the error in the momentum component looks similar to the ones seen in figure \ref{fig:two-points-all-the-same}.
...
The resulting plots are displayed in figure \ref{fig:stiffness-analysis-order-1-standard-values-h1}. As you can see in these plots, the error terms $SE_x^1$ and $SE_y^3$ are 0.
...
The fact that all $y$ momentum errors are zero deserves a special attention. However, we will explain this fact in a more generalized setting --- we won't restrict ourselves to second order and will assume that all heights are equal (not exactly 10).
...
From equations (\ref{eq:1}) and (\ref{eq:2}) we can see the reason that we can choose the $y$ momentums as we like it --- it won't influence the error term $SE_x^i$.
...
The error plots for the $y$ momentums are shown in figure \ref{fig:ord2_varying_ux1_uy2_1__ycomponent} and don't look as structured anymoore (compared to before where they were all 0).
...
We will now come back to only modifying the $x$ momentums. We will set them all to 1 and leave $u_{x,1}$ as axis variable. We expect parabolae with minima at $u_{x,1}=1$ (since all other $x$ momentums are 1). You can see the plots in figure \ref{fig:ord2_varying_ux1_ux2-ux6_1}.
