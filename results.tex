...
Uniqueness means, that $\sum_{i=1}^n \phi_i(x,y)$ is the only polynomial that has the property that it evaluates to 1 \emph{for each support point}. However, we know that the ``polynomial'' $f(x,y)=1$ is a polynomial that has this property as well. Uniqueness now tells us that they must be the same: $\sum_{i=1}^n \phi_i(x,y)=1$. 
...
Thus, we derived an approximation for an integral of the form 
\begin{equation*}
\int_T f(x,y)\, dT = \int_{0}^1 \int_0^{1-x} f(x,y)\, dy\, dx.
\end{equation*}
Therefore, we subdivide the triangle into similar sub-triangles such that all sub-triangles together cover the original triangle $T$. 
...
Of course, it is in most cases more appropriate to use more subtriangles. If we use four subtriangles (i.e. $r=2$), we get the following centers of the subtriangles: 
\begin{equation*}
\mathtt{CENTERS} = \left\{ (\frac{1}{6}, \frac{1}{6}), (\frac{1}{6}, \frac{1}{6}), (\frac{1}{3}, \frac{1}{6}), (\frac{1}{6},\frac{1}{3}) \right\}.
\end{equation*}
...
\todo{Erwähnen, dass sich x und y symmetrisch verhalten.}

\section{Singularities}
\label{sec:stiffness-analysis-singularities}

When we do a numerical integration, we of course want to know if this is useul at all. It may be the case that we have some singularities (w.r.t. $x$ and $y$) that do not allow us to compute the integral properly. Therefore, we have to ask when these singularities can occur.

Answering this quesion in general is a really challenging (if not impossible) task, so we will show te basic technique that can be used to answer specific questions.

Fist of all, note that singularities (w.r.t. $x$ and $y$) can only occur if tey are in the denominator of some fraction. If one examines the terms $SE_x$ and $SE_y$, it becomes obvious that we only might have singularities if

\begin{equation*}
  \sum_{j=1}^n h_j \phi_j = 0.
\end{equation*}

This means -- in particular -- that for singularities, only the heights are decisive. All the momentums do not influence them.

\todo{Genaue Gleichung zeigen und Deflücke aufzeigen.}

\subsection{Order 1}
\label{sec:stiffness-analysis-singularities-ord1}

We will now focus on a special case: We consider basis functions for order 1 (these can be seen in table \ref{tab:basis-functions-for-low-degrees}) and set all heights except $h_1$ to 10.

If we do so, we have to consider $h_{{1}} \left( 1-2\,y \right) +h_{{2}} \left( -1+2\,x+2\,y \right) +h_
{{3}} \left( 1-2\,x \right)$ with $h_2=h_3=10$.


\subsection{Order 2}
\label{sec:stiffness-analysis-singularities-ord2}




\section{How to make error 0}
\label{sec:stiffness-analysis-how-to-make-error-0}

As we will see later, it is advantageous if the heights resp. $x$-momentums resp. $y$-momentums do not differ very strongly. We will do a short computation to show this: Therefore, we will assume $h_1=h_2=\dots=h_n$ and $u_{x,1}=\dots=u_{x,n}$ and $u_{y,1}=\dots=u_{y,n}$ and compute the error term.

We will showcase some examples here: Let us start with the term $\frac{u_x\cdot u_y}{h}$ (a subterm of $SE$). We will compare the exact and the approximative solution for this subterm:

\begin{align*}
  \sum_{j=1}^n \left( \frac{u_{x,j}\cdot u_{y,j}}{h_j} \cdot \phi_j\right) - \frac{(\sum_{j=1}^n u_{x,i}\cdot\phi_j) \cdot (\sum_{j=1}^n u_{y,j}\cdot\phi_j)}{\sum_{j=1}^n h_j \phi_j} &= \\
  \sum_{j=1}^n \left( \frac{u_{x,1}\cdot u_{y,1}}{h_1} \cdot \phi_j\right) - \frac{(\sum_{j=1}^n u_{x,1}\cdot\phi_j) \cdot (\sum_{j=1}^n u_{y,1}\cdot\phi_j)}{\sum_{j=1}^n h_1 \phi_j} &= \\
  \frac{u_{x,1}\cdot u_{y,1}}{h_1} \cdot \sum_{j=1}^n \phi_j - \frac{u_{x,1} \cdot \sum_{j=1}^n \phi_j \cdot u_{y,1}\cdot \sum_{j=1}^n \phi_j}{ h_1 \cdot \sum_{j=1}^n \phi_j} &= \\
  \frac{u_{x,1}\cdot u_{y,1}}{h_1} - \frac{u_{x,1} \cdot u_{y,1}}{ h_1 } &= 0 \\
\end{align*}

Similarily, we get an error value of 0 for the term $\frac{u_x^2}{h}+\frac{1}{2}g h^2$.\todo{Ausführlich errechnen.}
